...
Generator 函数的语法糖，`async`函数就是将 Generator 函数的星号。

（`*`）替换成`async`，将`yield`替换成`await`。`async`函数完全可以看作多个异步操作，包装成的一个 Promise 对象。
...
// 由于`async`函数返回的是 Promise 对象，上面也可以写成
